  ...
  Future<List<Map<String, dynamic>>> fetchCoupons() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.163.176:8000/api/coupons/'),
        headers: {
...
Future<List<Map<String, dynamic>>> fetchClaimedCoupons() async {
  try {
    ...
    final response = await http.get(
      Uri.parse('http://192.168.163.176:8000/api/coupons/inventory'),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
...
Future<void> redeemCoupon(String couponCode) async {
  try {
    ...
    final response = await http.post(
      Uri.parse('http://192.168.163.176:8000/api/coupons/$couponCode/redeem'), // Using couponCode in the URL
      headers: {
  ...
  Future<List<Map<String, dynamic>>> fetchRedeemedProducts() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.163.176:8000/api/coupons/redeemed'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
